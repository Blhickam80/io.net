...
- ✅ **Podium/Birdeye and No-Show Calculator FAQ reinforcement**: `compare/podium-vs-birdeye-vs-nicejob.html` gets a new FAQ schema entry + visible FAQ for "best reputation management software for small business" and "Podium pricing"; `tools/no-show-calculator.html` gets its first FAQPage schema + visible FAQ for "appointment no show rate by industry" (with an honest range, not a fabricated single stat). This closes out the FAQ-reinforcement pass across all remaining candidate pages named in the weekly review.
...
1. **Check Votel.ai's own referral dashboard for early click/signup data** — doesn't require new tooling, just a login; would be the first real signal on which page/placement is actually driving affiliate interest, even before formal analytics exists.
...
4. **HVAC / plumbing / law-firm industry pages** — seeded as "planned" keywords in `seo/keywords.csv`; hold until the existing roofing/dental/real-estate pages show some real traffic signal, so we're not building the same page type 6x on pure speculation.
