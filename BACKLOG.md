...
0. **Post the rendered Short 01 video to https://www.youtube.com/@yardwick** — a real captioned video (not just a script) was generated by driving the actual live calculator with Playwright and burning in captions with ffmpeg; delivered directly to the operator. Just needs upload + the caption text from `video/scripts/shorts-01-missed-call-hook.md`. This is now the single highest-priority not-blocked action.
0b. **Generate shorts-02 and shorts-03 the same way** using `automation/record_calculator_short.py` (parameterized: pass --calls/--missed/--close/--ticket/--recall/--hook/--cta) once shorts-01's real-world performance gives a read on whether this format works — or generate them now in parallel if there's no reason to wait.
