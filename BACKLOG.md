...
1. **Best AI Receptionist for Real Estate Agents** — 3rd industry page, same template.
...
3. **Write the GoHighLevel review/overview page** — foundational money page other content links into.
...
5. **Write 3-5 YouTube Short scripts demoing the missed-call calculator + one AI receptionist product via public demo videos/screenshots** — Engine D/E seed content, ready to record once a channel exists (ACCESS_NEEDED #6).
6. **SEO keyword list build-out** — expand `seo/keywords.csv` beyond the seed list (see SEO_PLAN.md) using free tools (Google autosuggest, AlsoAsked-style manual research, competitor sitemaps).
7. **Add GitHub Actions workflow to auto-deploy `website/public` to GitHub Pages** so the site goes live the moment Pages is enabled (removes a manual step from ACCESS_NEEDED #3).
